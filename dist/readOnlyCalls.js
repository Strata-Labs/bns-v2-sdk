...
async function getLastTokenId({ network }) {
  try {
    ...
    throw new Error(
      `Unexpected Clarity Value type: ${(0, transactions_1.getCVTypeString)(
        responseCV
      )}`
    );
  }
...
async function canResolveName({ fullyQualifiedName, network }) {
  try {
    ...
    if (
      responseCV.type === transactions_1.ClarityType.ResponseOk &&
      ...
      if (
        renewalCV.type === transactions_1.ClarityType.UInt &&
  ...
  try {
    ...
    if (responseCV.type === transactions_1.ClarityType.ResponseOk) {
      if (responseCV.value.type === transactions_1.ClarityType.OptionalSome) {
        if (
          responseCV.value.value.type ===
            transactions_1.ClarityType.PrincipalStandard ||
          responseCV.value.value.type ===
            transactions_1.ClarityType.PrincipalContract
        ) {
...
async function fallbackContractCall(name, namespace, network) {
  const randomAddress = (0, utils_1.generateRandomAddress)();
  debug_1.debug.log("Falling back to contract call:", {
    name,
    namespace,
    network,
  });
...
async function getNamespacePrice({ namespace, network }) {
  ...
  return (0, callersHelper_1.bnsV2ReadOnlyCall)({
    ...
    if (responseCV.type === transactions_1.ClarityType.ResponseOk) {
      if (
        responseCV.value.type === transactions_1.ClarityType.Int ||
    ...
    .then((responseCV) => {
      if (responseCV.type === transactions_1.ClarityType.ResponseOk) {
        ...
        if (responseOkValue.type === transactions_1.ClarityType.ResponseOk) {
          ...
          if (
            nestedResponseOkValue.type === transactions_1.ClarityType.Int ||
          ...
          responseOkValue.type === transactions_1.ClarityType.Int ||
  ...
  return (0, callersHelper_1.bnsV2ReadOnlyCall)({
    ...
    if (responseCV.type === transactions_1.ClarityType.ResponseOk) {
      if (responseCV.value.type === transactions_1.ClarityType.BoolTrue) {
        ...
        responseCV.value.type === transactions_1.ClarityType.BoolFalse
      ) {
        return false;
      } else {
  ...
  try {
    ...
    if (
      ...
      return {
        ...
        properties: {
          "namespace-manager":
            properties["namespace-manager"].type ===
            transactions_1.ClarityType.OptionalNone
              ? null
              : (0, transactions_1.cvToString)(
                  properties["namespace-manager"].value
                ),
          "manager-transferable":
            properties["manager-transferable"].type ===
            transactions_1.ClarityType.BoolTrue,
          "manager-frozen":
            properties["manager-frozen"].type ===
            transactions_1.ClarityType.BoolTrue,
          "namespace-import": (0, transactions_1.cvToString)(
            properties["namespace-import"]
          ),
          "revealed-at": properties["revealed-at"].value,
          "launched-at":
            properties["launched-at"].type ===
            transactions_1.ClarityType.OptionalNone
              ? null
              : properties["launched-at"].value.value,
          lifetime: properties["lifetime"].value,
          "can-update-price-function":
            properties["can-update-price-function"].type ===
            transactions_1.ClarityType.BoolTrue,
          "price-function": (0, utils_1.parsePriceFunction)(
            properties["price-function"].value
          ),
        },
      };
    }
...
async function getNameInfo({ fullyQualifiedName, network }) {
  try {
    ...
    if (
      responseCV.type === transactions_1.ClarityType.OptionalSome &&
      responseCV.value.type === transactions_1.ClarityType.Tuple
    ) {
      ...
      return {
        ...
        importedAt:
          properties["imported-at"].type ===
          transactions_1.ClarityType.OptionalNone
            ? null
            : properties["imported-at"].value.value,
        preorderedBy:
          properties["preordered-by"].type ===
          transactions_1.ClarityType.OptionalNone
            ? null
...
async function fetchUserOwnedNames({ senderAddress, network }) {
  try {
    ...
    while (true) {
      ...
      const assets = response.data.results.map((asset) =>
