...
async function buildTransferNameTx({
  fullyQualifiedName,
  newOwnerAddress,
  senderAddress,
  network,
  ...
  const { subdomain, namespace, name } = (0, utils_1.decodeFQN)(
    fullyQualifiedName
  );
  if (subdomain) {
  ...
  const postConditionSender = transactions_1.Pc.principal(senderAddress)
    .willSendAsset()
    .nft(
      `${(0, config_1.getBnsContractAddress)(network)}.${
        config_1.BnsContractName
      }::BNS-V2`,
      (0, transactions_1.uintCV)(id)
    );
...
async function buildListInUstxTx({
  id,
  price,
  ...
  return {
    ...
    functionArgs: [
      ...
      (0, transactions_1.contractPrincipalCV)(
        commissionTraitAddress,
        commissionTraitName
      ),
    ],
...
async function buildBuyInUstxTx({
  id,
  expectedPrice,
  commissionTraitAddress,
  commissionTraitName,
...
async function buildSetPrimaryNameTx({
  fullyQualifiedName,
  senderAddress,
  network,
}) {
...
async function buildPreorderNamespaceTx({
  namespace,
  salt,
  stxToBurn,
  senderAddress,
  ...
  const saltedNamespaceBytes = (0, common_1.utf8ToBytes)(
    `${namespace}.${salt}`
  );
  const hashedSaltedNamespace = (0, transactions_1.hash160)(
    saltedNamespaceBytes
  );
  ...
  return {
    ...
    functionName: bnsFunctionName,
    functionArgs: [
      (0, transactions_1.bufferCV)(hashedSaltedNamespace),
...
async function buildRevealNamespaceTx({
  namespace,
  salt,
  priceFunction,
  lifetime = 0n,
  ...
  if (namespaceManagerAddress) {
    if (namespaceManagerAddress.includes(".")) {
      ...
      managerAddressCV = (0, transactions_1.someCV)(
        (0, transactions_1.contractPrincipalCV)(contractAddress, contractName)
      );
    } else {
      managerAddressCV = (0, transactions_1.someCV)(
        (0, transactions_1.standardPrincipalCV)(namespaceManagerAddress)
      );
    }
  } else {
...
async function buildTurnOffManagerTransfersTx({
  namespace,
  senderAddress,
  network,
}) {
...
async function buildImportNameTx({
  namespace,
  name,
  beneficiary,
  senderAddress,
...
async function buildNamespaceUpdatePriceTx({
  namespace,
  priceFunction,
  senderAddress,
  network,
...
async function buildNamespaceFreezePriceTx({
  namespace,
  senderAddress,
  network,
}) {
...
async function buildNameClaimFastTx({
  fullyQualifiedName,
  stxToBurn,
  sendTo,
  senderAddress,
  ...
  const { namespace, name, subdomain } = (0, utils_1.decodeFQN)(
    fullyQualifiedName
  );
  if (subdomain) {
...
async function buildPreorderNameTx({
  fullyQualifiedName,
  salt,
  stxToBurn,
  senderAddress,
  ...
  return {
    ...
    functionName: bnsFunctionName,
    functionArgs: [
      (0, transactions_1.bufferCV)(hashedSaltedName),
      (0, transactions_1.uintCV)(stxToBurn),
    ],
    postConditions,
    network,
...
async function buildRegisterNameTx({
  fullyQualifiedName,
  salt,
  stxToBurn,
  senderAddress,
  ...
  let postConditions = [];
  if (stxToBurn > 0) {
    const burnSTXPostCondition = transactions_1.Pc.principal(
      `${(0, config_1.getBnsContractAddress)(network)}.${
        config_1.BnsContractName
      }`
    )
...
async function buildPreviousRegisterNameTx({
  fullyQualifiedName,
  salt,
  stxToBurn,
  senderAddress,
  ...
  const transferNFTPostCondition = transactions_1.Pc.principal(currentOwner)
    ...
    .nft(
      ...
      (0, transactions_1.uintCV)(nameId)
    );
...
async function buildClaimPreorderTx({
  fullyQualifiedName,
  salt,
  stxToClaim,
  senderAddress,
  ...
  const saltedNamesBytes = (0, common_1.utf8ToBytes)(
    `${fullyQualifiedName}${salt}`
  );
  const hashedSaltedName = (0, transactions_1.hash160)(saltedNamesBytes);
  const returnSTXPostCondition = transactions_1.Pc.principal(
    `${(0, config_1.getBnsContractAddress)(network)}.${
      config_1.BnsContractName
    }`
  )
...
async function buildRenewNameTx({
  fullyQualifiedName,
  stxToBurn,
  senderAddress,
  network,
...
async function buildUpdateZonefileTx({
  fullyQualifiedName,
  zonefileInputs,
  senderAddress,
  network,
  ...
  if (zonefileInputs) {
    ...
    zonefileCV = (0, transactions_1.someCV)(
      (0, transactions_1.bufferCVFromString)(zonefileString)
    );
  } else {
...
async function buildUpdateZonefileFlexibleTx({
  fullyQualifiedName,
  zonefileData,
  senderAddress,
  network,
...
async function buildUpdateZonefileFormattedTx({
  fullyQualifiedName,
  zonefileData,
  senderAddress,
  network,
  ...
  const bnsFunctionName = "update-zonefile";
  const { name, namespace } = (0, utils_1.decodeFQN)(fullyQualifiedName);
  const formattedZonefileData = (0, utils_1.createFormattedZonefileData)(
    zonefileData
  );
  let zonefileCV;
